...
exports.uploadPtdData = async (req, res) => {
    try {
        ...
        console.log("🚀 [PTD]: Processing file and detecting latest period...");
        const workbook = xlsx.readFile(req.file.path, { cellDates: true });
        const sheetNames = workbook.SheetNames;
        
        let affectedLoas = new Set();
        let detectedPeriods = []; // 🔥 Use array to store numeric values for sorting
        const batchSize = 500;
        ...
        if (sheetNames.includes('CJI5')) {
            ...
            const cji5Rows = cji5Data.filter(row => row['WBS Element']).map(row => {
                ...
                // No period collection from CJI5 as requested
                return [
        ...
        if (sheetNames.includes('CJ74')) {
            ...
            const cj74Rows = cj74Data.filter(row => row['Object']).map(row => {
                ...
                // 🔥 Collect Numeric Periods from CJ74
                const pValRaw = row['Per'] || row['per'] || row['PER'];
                if (pValRaw) {
                    // Extract only numbers (handle cases like 'P007' or just '7')
                    const pNum = parseInt(pValRaw.toString().replace(/[^0-9]/g, ''));
                    if (!isNaN(pNum)) detectedPeriods.push(pNum);
                }
            ...
            console.log("✅ [PTD]: CJ74 Data uploaded.");
        }
        ...
        if (loaList.length > 0) {
            ...
            console.log("✅ [PTD]: Dashboard Synced.");
        }

        // ==========================================
        // --- 4. 🔥 SINGLE MAIL TRIGGER (LATEST PERIOD ONLY) ---
        // ==========================================
        if (detectedPeriods.length > 0) {
            try {
                // Find the LATEST (MAX) period number
                const maxPeriod = Math.max(...detectedPeriods);
                const periodCode = `P${maxPeriod.toString().padStart(3, '0')}`; // e.g. P008

                console.log(`📧 [PTD]: Triggering ONE mail for latest period: ${periodCode}`);
                
                const recipients = ["neha.sain.ext@nokia.com"];
                await mailService.sendPTDUpdateAlert(recipients, periodCode);
                
                console.log(`✅ [PTD]: Single mail sent for ${periodCode}.`);
            } catch (mailErr) {
                console.error("❌ [PTD]: Mail failed:", mailErr.message);
            }
        } else {
            console.log("⚠️ [PTD]: No period found in CJ74. Skipping mail.");
        }

        triggerAutoSync('ptd_uploaded');
        return res.status(200).json({ message: "Everything Uploaded and Team Notified (Single Mail)!" });

    } catch (error) { 
        console.error("❌ [PTD] CRITICAL ERROR:", error.message); 
...
// console.log("==========================================");
// console.log("📧 PTD MAILER: Starting...");
// console.log("📧 Detected periods:", Array.from(detectedPeriods));
// console.log("==========================================");
...
//     periodArray.push(`P${curMonthNum.toString().padStart(3, '0')}`);

//     console.log("⚠️ No period detected from Excel.");
//     console.log("⚠️ Using fallback period:", periodArray[0]);
// }

// console.log("📧 Final periodArray:", periodArray);

// try {

//     const testingRecipient = ["neha.sain.ext@nokia.com"];

//     console.log("📧 Recipient:", testingRecipient);

//     for (const p of periodArray) {

//         console.log(`📧 About to call sendPTDUpdateAlert() for ${p}`);

//         const result = await mailService.sendPTDUpdateAlert(
//             testingRecipient,
//             p
//         );

//         console.log(`✅ transporter.sendMail() returned for ${p}`);
